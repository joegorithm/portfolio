...
const pages = {
    ...
    "projects/age-in-days-calculator": {
        title: "Age in Days Calculator"
    },
    "projects/timer": {
        title: "Timer"
    },
    "projects/castleguessr": {
        title: "CastleGuessr"
    },
};
...
Array.from(document.getElementsByTagName("header")).forEach((header) => {
    ...
    if (pathSegments.length > 0) {
        html = `
            <h1 class="page-title">
                ...
                ${pathSegments.slice(0, -1).map((segment, idx) => `
                        <a class="page-title-parent" href="/${segment}">${toTitleCase(segment)}</a>
                        <p class="page-title-separator">/</p>
                `).join('')}

                <a class="page-title-current" href="/${pathSegments.join('/')}">${(pages && pages[pathSegments.join('/')] && pages[pathSegments.join('/')].title) ? pages[pathSegments.join('/')].title : toTitleCase(pathSegments[pathSegments.length - 1] || '')}</a>
                <p class="page-title-current-simple">${(pages && pages[pathSegments.join('/')] && pages[pathSegments.join('/')].title) ? pages[pathSegments.join('/')].title : toTitleCase(pathSegments[pathSegments.length - 1] || '')}</p>
    ...
    // WARNING: Directly setting innerHTML can introduce XSS vulnerabilities if html contains unsanitized user input.
    // Ensure all dynamic content is sanitized before assigning to innerHTML.
